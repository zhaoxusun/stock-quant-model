...
def clean():
    ...
    # 5. Remove xgboost from sitepkgs (only keep xgb_pkgs copy with gzip/patch)
    for _xg_item in ('xgboost',):
        _xg_path = os.path.join(sitepkgs, _xg_item)
        if os.path.isdir(_xg_path) or os.path.isfile(_xg_path):
            saved = rm(_xg_path)
            if saved:
                total_saved += saved
                print(f'  Removed xgboost from sitepkgs ({saved/1e6:.1f} MB)')
    for _dist in os.listdir(sitepkgs):
        if _dist.startswith('xgboost-'):
            saved = rm(os.path.join(sitepkgs, _dist))
            if saved:
                total_saved += saved
                print(f'  Removed {_dist} ({saved/1e6:.1f} MB)')
    # scipy: remove if present (requirements may or may not include it)
